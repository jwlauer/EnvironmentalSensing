...
p1 = machine.Pin(11, machine.Pin.IN, machine.Pin.PULL_UP)
...
#set pin 11 to look for tips    
p1.irq(trigger=Pin.IRQ_RISING, handler=handleInterrupt)
#****************************************************************************   
...
if reset_cause == 1: #machine.PWRON_RESET:
    ...
    print('Woke due to deep sleep so may be incrementing total')
    if wake_reason == 4: #machine.TIMER_WAKE
        print('Woke because timer woke machine, not incrementing total')
    else:
...
#    f.write('%s/%s/%s,%s:%s:%s,%s,%s,%s,%s\r\n' % (datetime[0], datetime[1], datetime[2], datetime[4], datetime[5], datetime[6], str(total), battery, reset_cause, wake_reason))
#    f.close()
f = open('rainlog.txt', 'a')
f.write('%s/%s/%s,%s:%s:%s,%s,%s,%s,%s\r\n' % (datetime[0], datetime[1], datetime[2], datetime[4], datetime[5], datetime[6], str(total), battery, reset_cause, wake_reason))
f.close()
...
#esp32.wake_on_ext1([p1], level = esp32.WAKEUP_ALL_LOW)
